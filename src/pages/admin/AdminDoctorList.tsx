...
import {
  ...
  useAdminDeleteDoctor,
} from "@/queries";
import type { AdminDoctorUpdate } from "@/types";
import { toast } from "react-toastify";

export default function AdminDoctorList() {
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [editingDoctor, setEditingDoctor] = useState<AdminDoctorUpdate | null>(
    null
  );
  ...
  const { mutate: updateDoctor } = useAdminUpdateDoctor();
  const { mutate: deleteDoctor } = useAdminDeleteDoctor();
  ...
  const handleEditDoctor = (doctor: AdminDoctorUpdate) => {
