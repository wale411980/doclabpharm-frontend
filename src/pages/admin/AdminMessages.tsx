...
import { useAdminConversation, useAdminConversationMessages, useAdminMessagesStats, useAdminDeleteConversation, useAdminDeleteMessage } from "@/queries"
import type { AdminConversations, AdminConversationsMessages } from "@/types"
import { format, isToday, isYesterday, parseISO } from "date-fns"

export default function HealthcareMessaging() {
  const [selectedContact, setSelectedContact] = useState<AdminConversations | null>(null)
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedMessages, setSelectedMessages] = useState<Set<number>>(new Set())
  const [isSelectingMessages, setIsSelectingMessages] = useState(false)
  const [messageToDelete, setMessageToDelete] = useState<number | null>(null)
  const [conversationToDelete, setConversationToDelete] = useState<number | null>(null)  
  const [, setMessagesList] = useState<AdminConversationsMessages[]>([])

  const { data: stats, isLoading } = useAdminMessagesStats()
  const { data: conversations, refetch: refetchConversation } = useAdminConversation()
  const { data: conversationMessages, isLoading: isMessagesLoading, refetch: refetchMessages } = useAdminConversationMessages(selectedContact?.id ?? 0)
  const { mutate: deleteMessage } = useAdminDeleteMessage()
  const { mutate: deleteConversation } = useAdminDeleteConversation()

  const filteredContacts = conversations?.filter((conversation: AdminConversations) => {
  ...
  const messagesListed: AdminConversationsMessages[] = (conversationMessages ?? []).map((msg) => ({
    id: msg.id,
  ...
  function groupMessagesByDate(messages: AdminConversationsMessages[]) {
    const grouped: Record<string, AdminConversationsMessages[]> = {}
              ...
              <CardContent className="p-0 h-full flex flex-col">
                ...
                {/* Contacts List */}
                 <ScrollArea className="flex-1 overflow-y-auto max-h-[calc(100vh-300px)] px-2">
                  <div className="p-2">
                    {filteredContacts?.map((conversation: AdminConversations) => (
                      <div
                        key={conversation.id}
