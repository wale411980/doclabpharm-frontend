...
import {
  useAdminAllLabAppointments,
  useAdminReportsAdd,
  useAdminReportsUpdate,
} from "@/queries";
import type { AdminLabAppointments } from "@/types";
import { format, isSameDay } from "date-fns";
import { toast } from "react-toastify";
import ImageUpload from "@/components/ImageUpload";

export default function AdminLabAppointments() {
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<AdminLabAppointments | null>(null);
  ...
  const { data: allAppointments, isLoading } = useAdminAllLabAppointments();
  const { mutate: addReport } = useAdminReportsAdd();
  const { mutate: updateReport } = useAdminReportsUpdate();
            ...
            <div className="block md:hidden">
              <div className="space-y-4 p-4">
                {paginatedAppointments?.map(
                  (appointment: AdminLabAppointments) => (
                ...
                onClick={() => {
                  ...
                  mutationFn(mutationArgs, {
                    ...
                    onError: (error: any) => {
                      ...
                      toast.error(
                        isEditing ? "Update failed." : "Failed to add report. " + errMsg
                      );
                    },
                  });
                }}
