...
import type { AdminLabUpdate } from "@/types";
import { toast } from "react-toastify";

export default function AdminLabList() {
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [editingLab, setEditingLab] = useState<AdminLabUpdate | null>(null);
  ...
  const { data: labList, refetch, isLoading: loadingLabs } = useAdminLabList();
  const { mutate: updateLab } = useAdminUpdateLab();
  const { mutate: deleteLab } = useAdminDeleteLab();
  ...
  const handleEditLab = (lab: AdminLabUpdate) => {
    setEditingLab(lab);
    setEditForm({
