...
import {
  useSuperAdminPharmacyList,
  useSuperAdminUpdatePharmacy,
  useSuperAdminDeletePharmacy,
} from "@/queries";
import type {
  SuperAdminPharmacyUpdate,
  EditableSuperAdminPharmacy,
} from "@/types";
import { toast } from "react-toastify";

export default function SuperAdminPharmacyList() {
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [editingPharmacy, setEditingPharmacy] =
    useState<EditableSuperAdminPharmacy | null>(null);
  ...
  const { mutate: updatePharmacy } = useSuperAdminUpdatePharmacy();
  const { mutate: deletePharmacy } = useSuperAdminDeletePharmacy();
  ...
  const handleEditPharmacy = (
    pharmacy: SuperAdminPharmacyUpdate & { id: string | number }
  ) => {
