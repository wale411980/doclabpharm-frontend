...
import {
  useSuperAdminConsultationList,
  useSuperAdminGetConsultation,
  useSuperAdminAddConsultation,
  useSuperAdminUpdateConsultation,
  useSuperAdminDeleteConsultation,
} from "@/queries";
import type {
...
export default function SuperAdminConsultation() {
  const [, setServices] = useState<SuperAdminConsultation[]>([]);
  ...
  const [selectedService, setSelectedService] =
    useState<SuperAdminConsultation | null>(null);
  ...
  const { data: consultationById, isLoading: isConsultationLoading } =
    useSuperAdminGetConsultation(viewServiceId ?? undefined);

  const { mutate: addConsultation, isPending: isAddPending } =
    useSuperAdminAddConsultation();
  const { mutate: updateConsultation, isPending: isUpdatePending } =
    useSuperAdminUpdateConsultation();
  const { mutate: deleteConsultation, isPending: isDeletePending } =
    useSuperAdminDeleteConsultation();
  ...
  const handleCreate = () => {
    const newServiceData: SuperAdminAddConsultation = {
      name: formData.name,
    ...
    addConsultation(
      { data: newServiceData },
      {
        ...
        onError: (error: any) => {
          ...
          toast.error("Failed to create consultation. " + errMsg);
        },
      }
    );
  };
  ...
  const handleEdit = () => {
    ...
    const payload: SuperAdminAddConsultation = {
      name: formData.name,
      details: formData.details,
      price: formData.price,
      includeDetails: formData.includeDetails,
    ...
    updateConsultation(
      ...
      },
      {
        ...
        onError: (error: any) => {
          const errMsg = error?.response?.data?.message;
          toast.error("Failed to update consultation. " + errMsg);
        },
      }
    );
  };
  ...
  const handleDelete = () => {
    ...
    deleteConsultation(
      ...
      {
        onSuccess: () => {
          // Optionally remove it from local list if needed
          setServices((prev) =>
        ...
        onError: (error: any) => {
          const errMsg = error?.response?.data?.message;
          toast.error("Failed to delete consultation. " + errMsg);
        },
      }
    );
  };

  const openEditModal = (service: SuperAdminConsultation) => {
  ...
  const openDeleteDialog = (service: SuperAdminConsultation) => {
    setSelectedService(service);
        ...
        <DialogContent className="w-[95vw] max-w-[500px] max-h-[90vh] overflow-y-auto mx-auto">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            <Button
              ...
              disabled={!isFormValid || isAddPending || isUpdatePending}
            >
              {isCreateModalOpen
                ? isAddPending
                  ? "Creating..."
      ...
      >
        <AlertDialogContent className="w-[95vw] max-w-md mx-auto">
          ...
          <AlertDialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            >
              {isDeletePending ? "Deleting..." : "Delete Service"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
