...
import { useSuperAdminOrderHistories } from "@/queries";
import type { SuperAdminOrderHistory } from "@/types";

export default function SuperAdminOrderHistories() {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedOrder, setSelectedOrder] =
    useState<SuperAdminOrderHistory | null>(null);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);

  // Destructuring `data` from the query result
  const { data, isLoading, isError } = useSuperAdminOrderHistories(currentPage);
  ...
  const handleViewOrder = (order: SuperAdminOrderHistory) => {
          ...
          <div className="rounded-lg border border-green-200 bg-white overflow-hidden">
            ...
            <div className="hidden md:block">
              <Table>
                ...
                <TableBody>
                  {filteredOrders.map((order: SuperAdminOrderHistory) => (
