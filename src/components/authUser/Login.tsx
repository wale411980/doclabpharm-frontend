...
import { useLogin, useForgotPassword } from "@/queries/use-auth";
...
export function LoginModal({
  ...
  const { login } = useAuth();
  const { mutateAsync: logined } = useLogin();
  ...
  // 2. Send login credentials + device token to backend
  const handleLogin = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await logined({ email, password });
      ...
      // ✅ Check for pending booking
      const pendingBookingConsultation = localStorage.getItem(
        "PendingBookingConsultation"
      );

      // ✅ Check for test booking
      const testBooking = localStorage.getItem("testBooking");

      // Redirect accordingly
      if (user?.status === "unverified") {
        navigate("/user/verify-email");
      } else if (pendingBookingConsultation) {
