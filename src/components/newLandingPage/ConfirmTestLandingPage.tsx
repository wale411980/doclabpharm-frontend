...
export default function ConfirmTestLandingPage() {
  ...
  const pendingBooking = JSON.parse(
    localStorage.getItem("testBooking") || "{}"
  );
  ...
  // Extract test data
  const testId = Number(pendingBooking.id) || 0;
  const testName = pendingBooking.name || "";
  const testPrice = Number(pendingBooking.price) || 0;
  ...
  const handleAddToCart = () => {
    ...
    if (
      ...
      alert("Some booking information is missing.");
      return;
    }
    ...
    addItem({
      service_type: "lab",
      service_id: testId, // <- from test.id
  ...
  const handlePrevious = () => {
    ...
    navigate(`/patient/bookings/select-date-time?${params.toString()}`);
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-4xl mx-auto  pb-40 pt-40">
